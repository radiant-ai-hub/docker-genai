...
- [Installing the RSM-MSBA-GENAI-INTEL computing environment on Linux](#installing-the-rsm-msba-genai-intel-computing-environment-on-linux)
- [Updating the RSM-MSBA-GENAI-INTEL computing environment on Linux](#updating-the-rsm-msba-genai-intel-computing-environment-on-linux)
...
## Installing the RSM-MSBA-GENAI--INTEL computing environment on Linux (Ubuntu 24.04)

Please follow the instructions below to install the rsm-msba-genai-intel computing environment. This is a slimmed-down image that has Python, Postgres, UV, Quarto, and a few other tools installed. The computing environment will be consistent across all students and faculty, easy to update, and also easy to remove if desired (i.e., there will *not* be dozens of pieces of software littered all over your computer).
...
source <(curl -s https://raw.githubusercontent.com/radiant-ai-hub/docker-genai/main/install/install-docker.sh);
```
...
cp -p ~/git/docker-genai/launch-rsm-msba-genai-intel.sh ~/Desktop;
~/Desktop/launch-rsm-msba-genai-intel.sh;
```
...
The code above also creates a copy of the file `launch-rsm-msba-genai-intel.sh` on your Desktop that you can use to start the container again in the future.
...
## Updating the RSM-MSBA-GENAI-INTEL computing environment on Linux
...
To save (or commit) these changes so they *will* be present after a (container) restart type, for example, `c myimage` (+ Enter). This creates a new docker image with your changes and also a new launch script on your Desktop with the name `launch-rsm-msba-genai-arm-myimage.sh` that you can use to launch your customized environment in the future.
...
You should always stop the `rsm-msba-genai-intel` docker container using `q` (+ Enter) in the launch menu. If you want a full cleanup and reset of the computational environment on your system, however, execute the following commands from a (bash) terminal to (1) remove prior R(studio) and Python packages, (2) remove all docker images, networks, and (data) volumes, and (3) 'pull' only the docker image you need (e.g., rsm-msba-genai-intel):
