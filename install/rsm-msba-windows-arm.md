...
After running the commands above you will be able to start the docker container by typing `~/git/docker-genai/launch-rsm-msba-genai-arm.sh -v ~` from an Ubuntu terminal.
...
if [ -n "$DTOP" ]; then
  echo "wt.exe wsl.exe ~/git/docker-genai/launch-rsm-msba-genai-arm.sh -v ~" > /mnt/c/Users/"$USERNAME$DTOP"/launch-rsm-msba.bat;
  ...
  echo "wt.exe wsl.exe ~/git/docker-genai/launch-rsm-msba-genai-arm.sh -v ~" > /mnt/c/Users/"$USERNAME"/launch-rsm-msba.bat;
...
```bash
wt.exe wsl.exe ~/git/docker-genai/launch-rsm-msba-genai-arm.sh -v ~
pause
```
...
## Updating the RSM-MSBA-GENAI-ARM computing environment on Windows
...
rm -rf ~/git/docker*;
git clone https://github.com/radiant-ai-hub/docker-genai.git ~/git/docker-genai;
cp -p ~/git/docker-genai/launch-rsm-msba-genai-arm.sh ~/Desktop/launch-rsm-msba.bat;
```
...
You should always stop the `rsm-msba-genai-arm` docker container using `q` (+ Enter) in the launch menu. If you want a full cleanup and reset of the computational environment on your system, however, execute the following commands from a (bash) terminal to (1) remove locally installed R and Python packages, (2) remove all docker images, networks, and (data) volumes, and (3) 'pull' only the docker image you need (e.g., rsm-msba-genai-arm):
